...
void ZdcTestAnalysis::update(const BeginOfJob * job) {
  //job
  std::cout<<" ZdcTestAnalysis:  beginning of job"<<std::endl;;
}
...
//    if (verbosity >= 2)
      std::cout << " ZdcTestAnalysis:  Step " << stepL << "," << stepE <<std::endl;
...
//	if (verbosity >= 2)
	  std::cout << " ZdcTestAnalysis:   GHD " << jj << ": " << theReplicaNumbers[jj] << ","
	...
	thePVtype = 0;
//	if (verbosity >= 2)
	  std::cout << " ZdcTestAnalysis: pvtype=0 hd=" << historyDepth << " " << theReplicaNumbers[0] << ","
...
//      if (verbosity >= 2)
	std::cout << " ZdcTestAnalysis:  hd=0 " << theReplicaNumber << "," 
		  << thePVname << "," << theLVname << "," 
		  << theMaterialName << std::endl;
    }
    else {
      std::cout << " ZdcTestAnalysis:  hd<0:  hd=" << historyDepth << std::endl;
    }
...
void ZdcTestAnalysis::update(const EndOfEvent * evt) {
  ...
  std::cout << "  ZdcTestAnalysis:  accessed all HC";
  
  int theZDCHCid = G4SDManager::GetSDMpointer()->GetCollectionID("ZDCHITS");
  std::cout << " ZdcTestAnalysis:  - theZDCHCid = " << theZDCHCid;
  
  CaloG4HitCollection* theZDCHC = (CaloG4HitCollection*) allHC->GetHC(theZDCHCid);
  std::cout << " ZdcTestAnalysis:  - theZDCHC = " << theZDCHC << std::endl;
  ...
  std::cout << " ZdcTestAnalysis:   theZDCHC has " << nentries << " entries" << std::endl;
...
//	    if (verbosity >= 1)
	      std::cout << " ZdcTestAnalysis:  entry #" << ihit << ": fiberID=0x" << std::hex 
	   ...
	   is!= energyInFibers.end(); is++)
	{
      ...
      std::cout << " ZdcTestAnalysis:  Event has " << nvertex << " vertex" << std::endl;
      if (nvertex==0)
	std::cout << " ZdcTestAnalysis:  End Of Event  ERROR: no vertex" << std::endl;
      
      for (int i = 0 ; i<nvertex; i++) {
	
	G4PrimaryVertex* avertex = (*evt)()->GetPrimaryVertex(i);
	if (avertex == 0)
	  std::cout << " ZdcTestAnalysis:  End Of Event ERR: pointer to vertex = 0"
		       << std::endl;
	std::cout << " ZdcTestAnalysis: Vertex number :" <<i << std::endl;
	int npart = avertex->GetNumberOfParticle();
	if (npart ==0)
	  std::cout << " ZdcTestAnalysis:  End Of Event ERR: no primary!" << std::endl;
	...
	if (pInit==0) {
	  std::cout << " ZdcTestAnalysis:  End Of Event  ERR: primary has p=0 " << std::endl;
	}
      } else {
	std::cout << " ZdcTestAnalysis:  End Of Event ERR: could not find primary "
		     << std::endl;
      }
  ...
  if (iEvt < 10)
    std::cout << "  ZdcTestAnalysis:  Event " << iEvt << std::endl;
