...
ZdcShowerLibrary::ZdcShowerLibrary(std::string & name, const DDCompactView & cpv,
  ...
  npe = 25; //I guess this is how we had the ZDC RPD
  hits.reserve(npe);
...
std::vector<ZdcShowerLibrary::Hit> & ZdcShowerLibrary::getHits(G4Step * aStep, bool & ok) {
  ...
  float xWidthRPD = fabs(theXRPDChannelBoundaries[0]- theXRPDChannelBoundaries[1]);
  float yWidthRPD = fabs(theYRPDChannelBoundaries[0]- theYRPDChannelBoundaries[1]);
  float zWidthRPD = 10; //I hope this means equal to 1cm thick
  ...
  for (int i = 0; i < npe; i++) {
    ...
    if((i> 4) && (i<21)){
      ...
      channel = i - 4;
      xxlocal = theXRPDChannelBoundaries[(i%4)-1]+(xWidthRPD/2.);;
      xx = xxlocal + X0;
      int bleh=(fabs(i-23))/5;
      yylocal = theYRPDChannelBoundaries[bleh]+(yWidthRPD/2.);
      // yylocal = theYRPDChannelBoundaries[(fabs(i-23))/5]+(yWidthRPD/2.);
      yy = yylocal + Y0;
      zzlocal = (hitPointOrig.z() > 0.) ?
      theZRPDChannelBoundaries+ (zWidthRPD/2.) : theZRPDChannelBoundaries- (zWidthRPD/2.);
      zz = (hitPointOrig.z() > 0.) ? zzlocal +  Z0 : zzlocal -  Z0;
      pos = G4ThreeVector(xx,yy,zz);
      posLocal = G4ThreeVector(xxlocal,yylocal,zzlocal);
    }
    ...
    if(i > 20){
      ...
      yy = yylocal + Y0;
      zzlocal = (hitPointOrig.z() > 0.) ? 
        theZHadChannelBoundaries[i-21] + (zWidthHAD/2.) : theZHadChannelBoundaries[i-21] - (zWidthHAD/2.);
      zz = (hitPointOrig.z() > 0.) ? zzlocal +  Z0 : zzlocal -  Z0; 
      pos = G4ThreeVector(xx,yy,zz);
      posLocal = G4ThreeVector(xxlocal,yylocal,zzlocal);
    }
...
int ZdcShowerLibrary::getEnergyFromLibrary(G4ThreeVector hitPoint, G4ThreeVector momDir, double energy,
  ...
  float fact = 0.;
/////////////////////////////////////////////////////////////
////////// BELOW LOOKS LIKE THE ACTUAL CONVERSION FACTORS ///
//////////  CHECK WHAT iparCode does and how it efffects ////
////////// the shower library, the ZdcSD.cc may also be /////
/////////// important///////////////////////////////////////
