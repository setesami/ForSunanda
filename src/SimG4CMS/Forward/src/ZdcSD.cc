...
void ZdcSD::getFromLibrary (G4Step* aStep) {
  ...
  */
  ...
  for (unsigned int i=0; i<hits.size(); i++) {
    ...
    std::cout<<"ForwardSim"<< "ZdcSD: Final Hit number:"<<i<<"-->"
			   ...
			   <<" New HitPosition: "<<posGlobal<<std::endl;
  }
...
double ZdcSD::getEnergyDeposit(G4Step * aStep, edm::ParameterSet const & p ) {
  ...
  if (aStep == NULL) {
    std::cout<<"ForwardSim" << "ZdcSD::  getEnergyDeposit: aStep is NULL!"<<std::endl;
    return 0;
  } else {
    ...
    if ((beta > bThreshold) && (charge != 0) && (nameVolume == "ZDC_EMFiber" || nameVolume == "ZDC_HadFiber" || nameVolume == "ZDC_FlowFiber")) {
      std::cout<<"ForwardSim"<< "ZdcSD::  getEnergyDeposit:  pass "<<std::endl; 
	...
	<< "," << NCherPhot<<std::endl;
      ...
      if (beta <= bThreshold)
        std::cout<<"ForwardSim"
	  << "ZdcSD::  getEnergyDeposit: fail beta=" << beta<<std::endl;
      if (charge == 0)
        std::cout<<"ForwardSim" 
	  << "ZdcSD::  getEnergyDeposit: fail charge=0"<<std::endl;
      if ( !(nameVolume == "ZDC_EMFiber" || nameVolume == "ZDC_HadFiber" || nameVolume=="ZDC_FlowFiber") )
        std::cout<<"ForwardSim" 
	  << "ZdcSD::  getEnergyDeposit: fail nv=" << nameVolume<<std::endl;
    }
...
void ZdcSD::setNumberingScheme(ZdcNumberingScheme* scheme) {
  if (scheme != 0) {
    std::cout<<"ForwardSim" << "ZdcSD: updates numbering scheme for " 
			       << GetName()<<std::endl;
    if (numberingScheme) delete numberingScheme;
...
int ZdcSD::setTrackID (G4Step* aStep) {
  ...
  if (primaryID == 0) {
//#ifdef DebugLog
    std::cout<<"ZdcSD" << "ZdcSD: Problem with primaryID **** set by force "
			<< "to TkID **** " << theTrack->GetTrackID()<<std::endl;
//#endif
    primaryID = theTrack->GetTrackID();
    }
